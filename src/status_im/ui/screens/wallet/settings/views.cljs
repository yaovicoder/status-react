...
(def hook
  ...
  {:properties
   ...
   :hook
   (reify hooks/Hook
     (hook-in [_ id {:keys [title view on-click]} {:keys [db]}]
       {:db (assoc-in db [:wallet :settings id] {:title title :view view :on-click on-click})})
...
(defview settings-hook []
  (letsubs [{:keys [title view]} [:get-screen-params :wallet-settings-hook]]
...
(defn- setting->action [{:keys [title on-click] :as m}]
  {:label  title
   :action #(do
              (when on-click
