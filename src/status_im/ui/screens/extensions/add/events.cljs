...
(re-frame/reg-fx
 ...
 (fn [{:keys [extensions follow-up]}]
   (doseq [{:keys [url active?]} extensions]
     (extensions/load-from url #(re-frame/dispatch [follow-up url (-> % extensions/read-extension extensions/parse) active?])))))

(handlers/register-handler-fx
 :extensions/stage
 (fn [{:keys [db] :as cofx} [_ _ extension-data]]
   (let [commands (extensions.registry/existing-commands cofx extension-data)]
     (if (empty? commands)
       (fx/merge cofx
                 {:db (assoc db :staged-extension extension-data)}
                 (navigation/navigate-to-cofx :show-extension nil))
       {:utils/show-popup {:title   (i18n/label :t/error)
                           :content (i18n/label :t/extension-commands-cannot-be-added
                                                {:commands (->> commands
                                                                (map name)
                                                                (clojure.string/join ", "))})}}))))
