  ...
  (letsubs [input-ref (reagent/atom nil)]
    ...
    [text-input/text-input-with-label
     {:style               styles/recovery-phrase-input
      ...
      :auto-correct        false
      :on-change-text      #(re-frame/dispatch [:accounts.recover/passphrase-input-changed (security/mask-data %)])
      :on-blur             #(re-frame/dispatch [:accounts.recover/passphrase-input-blured])
  ...
  [react/view {:style                       styles/password-input
   ...
   [text-input/text-input-with-label
    {:label             (i18n/label :t/password)
     ...
     :auto-focus        false
     :on-change-text    #(re-frame/dispatch [:accounts.recover/password-input-changed (security/mask-data %)])
     :on-blur           #(re-frame/dispatch [:accounts.recover/password-input-blured])
          ...
          [components.common/bottom-button
           {:forward?  true
            ...
            :on-press  (utils.core/wrap-call-once!
                        #(re-frame/dispatch [:accounts.recover/sign-in-button-pressed]))}]])])))
