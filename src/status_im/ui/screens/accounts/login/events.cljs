...
(handlers/register-handler-fx
 ...
 (fn [cofx [_ address photo-path name password]]
   (models/open-login-callback address photo-path name password cofx)))

(handlers/register-handler-fx
 :ui/login
 (fn [cofx [_ address]]
   (models/user-login address true cofx)))

(handlers/register-handler-fx
 :callback/login
 (fn [cofx [_ login-result address password save-password?]]
   (models/user-login-callback login-result address password save-password? cofx)))
