...
(fx/defn update-hooks
  ...
  (let [{:keys [dev-mode?] :as account} (get db :account/account)
        {:keys [active? hooks]} (get-in account [:extensions extension-key])]
    (apply utils/merge-fx cofx
           (mapcat (fn [[_ extension-hooks]]
                     (map (fn [[hook-id {:keys [hook-ref parsed]}]]
                            (if (and dev-mode? active?)
...
(fx/defn add-to-registry
  ...
  (let [{:keys [hooks]} extension-data
    ...
    (fx/merge cofx
              ...
              (update-hooks extension-key))))

(fx/defn remove-from-registry
  [{:keys [db] :as cofx} extension-key]
  (fx/merge cofx
            {:db (assoc-in db [:account/account :extensions extension-key :active?] false)}
            (update-hooks extension-key)
...
(defn existing-commands
  [cofx extension-data]
  (let [added-commands (reduce (fn [acc [_ {:keys [hooks] :as v}]]
                                 (->> (:commands hooks)
                                      (keys)
                                      (into acc)))
                               #{}
                               (get-in cofx [:db :account/account :extensions]))
        commands       (->> (get-in extension-data [:data :hooks :commands])
                            (keys)
                            (into #{}))]
    (set/intersection added-commands commands)))