...
(defn is-static-arr? [type]
  (let [ntypes (list-of-nested-types type)]
    (and ntypes (not (zero? (arr-size (last ntypes)))))))
...
(defn hex-to-value [hex type]
  (cond
    ...
    (str/starts-with? type "address") (str "0x" (subs hex (- 40 (count hex))))))
...
(defn dec-type [bytes]
  (fn [offset type]
    (cond
      ...
      (or (str/starts-with? type "bytes") (str/starts-with? type "string"))
