...
(fx/defn connect-success [{:keys [db now] :as cofx} {:keys [network on-success client-version]}]
    ...
    (if (ethereum/network-with-upstream-rpc? current-network)
      ...
      (fx/merge cofx
                {:ui/show-confirmation {:title               (i18n/label :t/close-app-title)
                                        ...
                                        :on-cancel           nil}}
                #(action-handler on-success {:network network :client-version client-version} %)))))

(defn connect-failure [{:keys [network on-failure reason]}]
  (action-handler on-failure
