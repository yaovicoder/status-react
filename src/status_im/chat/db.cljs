...
(defn- get-last-message
  [messages]
  (when messages
    (first (sort-by :timestamp > messages))))
...
(defn- enrich-active-chat
  [{:keys [chat-id messages unviewed-messages] :as chat}
   ...
   blocked-contacts]
  (let [filtered-messages-map (filter-messages-from-blocked-contacts messages
                                                                     blocked-contacts)
        filtered-messages (vals filtered-messages-map)
        unviewed-messages-count (count (set/intersection unviewed-messages
                                                         (set (keys filtered-messages-map))))
...
(defn keep-active-chats
  [chats contacts blocked-contacts dev-mode?]
  (keep (fn [{:keys [chat-id] :as chat}]
          (let [contact (get contacts chat-id)]
            (when (active-chat? chat contact dev-mode?)
              (enrich-active-chat chat contact blocked-contacts))))
        (vals chats)))
