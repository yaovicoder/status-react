...
(re-frame/reg-sub
 ...
 (fn [[contacts chats blocked-contacts {:keys [dev-mode?]}]]
   (chat.db/keep-active-chats chats contacts blocked-contacts dev-mode?)))
...
(re-frame/reg-sub
 ...
 :<- [::get-current-chat-messages-stream]
 :<- [:contacts/contacts]
 (fn [[chats {:keys [contacts chat-id] :as current-chat} messages all-contacts]]
