...
(re-frame/reg-sub
 ...
 (fn [[{:keys [messages]} blocked-contacts]]
   (chat.db/filter-messages-from-blocked-contacts messages blocked-contacts)))
...
(re-frame/reg-sub
 ...
 (fn [[contacts chats {:keys [dev-mode?]}]]
   (chat.db/keep-active-chats chats contacts dev-mode?)))
...
(re-frame/reg-sub
 ...
 (fn [[chats chat-id messages all-contacts]]
   (let [{:keys [contacts] :as current-chat} (get chats chat-id)
