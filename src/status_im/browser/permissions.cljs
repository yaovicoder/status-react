...
(fx/defn dispatch-next-permission-with-delay
  "If there are more than one permissions in a row we need a delay, to give some time for beautiful animation"
  [cofx dapp-name]
  {:dispatch-later [{:ms 1000 :dispatch [:browser.permissions.ui/next-permission-delay-dispatched dapp-name]}]})
...
(fx/defn allow-permission
  ...
  (let [{:keys [requested-permission message-id dapp-name]} (get-in db [:browser/options :show-permission])]
    (fx/merge (assoc-in cofx [:db :browser/options :show-permission] nil)
              ...
              (dispatch-next-permission-with-delay dapp-name))))
