...
(fx/defn resolve-ens-multihash
  [{{:keys [web3 network] :as db} :db} loading? error?]
  (when (and (not loading?) (not error?))
    (let [current-url (get-current-url (get-current-browser db))
      ...
      (if (ens? host)
        ...
        {:db (update db :browser/options assoc :url current-url :resolving? false)}))))
  ...
  (let [{:keys [history-index] :as browser} (get-current-browser (:db cofx))]
    (when (can-go-back? browser)
      (fx/merge cofx
                ...
                (resolve-ens-multihash false false)))))
  ...
  (let [current-url (get-current-url (get-current-browser db))
    ...
    (fx/merge cofx
              {:db (-> (update db :browser/options
                               assoc
                               :url (str "https://ipfs.infura.io/ipfs/"
                                         hash
                                         (subs current-url (+ (.indexOf current-url host) (count host))))
                               :resolving? false)
                       (assoc-in [:browser/options :resolved-ens host] (str "ipfs.infura.io/ipfs/" hash)))})))
  ...
  (let [options (get-in cofx [:db :browser/options])
    ...
    (when (and (not= "about:blank" url) (not= current-url url) (not= (str current-url "/") url))
      (let [resolved-ens (first (filter #(not= (.indexOf url (second %)) -1) (:resolved-ens options)))
            resolved-url (if resolved-ens (string/replace url (second resolved-ens) (first resolved-ens)) url)] (println "update-browser-on-nav-change" loading? error?)
           (fx/merge cofx
                     (update-browser-history browser resolved-url loading?)
                     (resolve-ens-multihash loading? error?))))))
