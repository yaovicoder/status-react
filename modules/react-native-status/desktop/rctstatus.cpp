...
void RCTStatus::startNode(QString configString) {
    ...
    jsonDoc = QString(QJsonDocument::fromVariant(configJSON).toJson(QJsonDocument::Compact));
    qDebug() << " RCTStatus::startNode updated configString: " << jsonDoc.toVariant().toMap();
    const char* result = StartNode(jsonDoc.toUtf8().data());
