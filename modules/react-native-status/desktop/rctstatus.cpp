...
void RCTStatus::verifyGroupMembershipSignatures(QString signatures, double callbackId) {
    ...
    QtConcurrent::run([&](QString signatures, double callbackId) {
            const char* result = VerifyGroupMembership(signatures.toUtf8().data());
            qDebug() << "RCTStatus::verifyGroupMembershipSignatures VerifyGroupMembership result: " << statusGoResultError(result);
            d->bridge->invokePromiseCallback(callbackId, QVariantList{result});
