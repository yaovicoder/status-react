...
void writeSingleLineLogFromJSServer(const QString &msg) {
  if (msg.startsWith("TRACE "))
    qCDebug(JSSERVER) << extractJSServerMessage(msg, 7);
  else if (msg.startsWith("DEBUG "))
    qCDebug(JSSERVER) << extractJSServerMessage(msg, 7);
  else if (msg.startsWith("INFO "))
    qCInfo(JSSERVER) << extractJSServerMessage(msg, 6);
  else if (msg.startsWith("WARN "))
    qCWarning(JSSERVER) << extractJSServerMessage(msg, 6);
  else if (msg.startsWith("ERROR "))
    qCWarning(JSSERVER) << extractJSServerMessage(msg, 7);
  else if (msg.startsWith("FATAL "))
    qCCritical(JSSERVER) << extractJSServerMessage(msg, 7);
  else
