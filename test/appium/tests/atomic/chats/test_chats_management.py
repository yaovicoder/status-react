...
from tests import marks, basic_user, camera_access_error_text
...
class TestChatManagement(SingleDeviceTestCase):
    ...
    def test_swipe_to_delete_1_1_chat(self):
        ...
        home.get_chat_with_user(users['M']['username']).swipe_and_delete()
        self.driver.close_app()
        self.driver.launch_app()
        sign_in.accept_agreements()
        sign_in.sign_in()
        if home.get_chat_with_user(users['M']['username']).is_element_displayed():
    ...
    def test_add_contact_by_pasting_public_key(self):
        ...
        start_new_chat.start_new_chat_button.click()
        if not start_new_chat.element_by_text(users['M']['username']).is_element_displayed():
    ...
    def test_incorrect_contact_code_start_new_chat(self):
        ...
        start_new_chat = home.plus_button.click()
        start_new_chat.start_new_chat_button.click()
        start_new_chat.public_key_edit_box.set_value(users['N']['public_key'][:-1])
        start_new_chat.confirm()
