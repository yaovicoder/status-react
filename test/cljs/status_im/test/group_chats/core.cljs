...
(deftest remove-member-test
  (with-redefs [utils.clocks/send inc]
    (testing "remove-member"
      (let [cofx {:current-public-key "me"
                  :db {:chats {chat-id {:admins #{"me"} :contacts #{"member"} :membership-updates [{:events [{:clock-value 1}]}]}}}}]
        (is (= {:chat-id chat-id
                :from "me"
                :events [{:type "member-removed"
                          :clock-value 2
                          :member "member"}]}
               (:group-chats/sign-membership (group-chats/remove-member cofx chat-id "me"))))))))
