...
def buildClojureScript(os) {
  sh 'make prod-build-desktop'
  if (os == 'MacOS') {
    sh 'PATH="/Users/administrator/qt/5.9.1/clang_64/bin:$PATH" ./scripts/build-desktop.sh buildClojureScript'
  } else {
...
def compile(os) {
  if (os == 'MacOS') {
    sh 'PATH="/Users/administrator/qt/5.9.1/clang_64/bin:$PATH" ./scripts/build-desktop.sh compile'
  } else {
...
def bundleMacOS(type = 'nightly') {
  ...
  sh 'PATH="/Users/administrator/qt/5.9.1/clang_64/bin:$PATH" ./scripts/build-desktop.sh bundle'
  dir(packageFolder) {
    withCredentials([
