...
;--------------------------------
  ...
  !addplugindir         ${top_srcdir}/deployment/windows
...
;--------------------------------
  ...
  OutFile "${top_srcdir}/Status-x86_64-setup.exe"
  
  SetCompressor /SOLID ZLIB

  ; Default installation folder
  InstallDir "$PROGRAMFILES64\Status.im"

  ; Get installation folder from registry if available
  InstallDirRegKey HKLM "Software\Status.Im\Status Desktop" ""
...
;--------------------------------
  ...
  VIAddVersionKey "OriginalFilename" "Status-x86_64-setup.exe"
...
;!define MEMENTO_REGISTRY_KEY "Software\Status.im\Status Desktop"
...
;Pages
  ...
  !define MUI_STARTMENUPAGE_REGISTRY_KEY "Software\Status.im\Status Desktop"
  ...
  !define MUI_FINISHPAGE_RUN_TEXT "&Start Status now"
...
Function ReadPreviousVersion
  ...
  ReadRegStr $PREVIOUS_INSTALLDIR HKLM "Software\Status.im\Status Desktop" ""

  Call CheckPrevInstallDirExists

  ${If} $PREVIOUS_INSTALLDIR != ""
    ; Detect version
    ReadRegStr $PREVIOUS_VERSION HKLM "Software\Status.im\Status Desktop" "Version"
    ${If} $PREVIOUS_VERSION != ""
      StrCpy $ALL_USERS 1
      SetShellVarContext all
      return
  ...
  ReadRegStr $PREVIOUS_INSTALLDIR HKCU "Software\Status.im\Status Desktop" ""

  Call CheckPrevInstallDirExists

  ${If} $PREVIOUS_INSTALLDIR != ""
    ; Detect version
    ReadRegStr $PREVIOUS_VERSION HKCU "Software\Status.im\Status Desktop" "Version"
    ${If} $PREVIOUS_VERSION != ""
      StrCpy $ALL_USERS 0
      SetShellVarContext current
      return
...
uac_tryagain:
    ...
    ${Switch} $0
    ${Case} 0
      ${IfThen} $2 = 1 ${|} Quit ${|} ;we are the outer process, the inner process has done its work, we are done.
...
Function .onInstSuccess
  ...
  ${If} $OldRunDir != ''
    ...
    ${GetFileVersion} "$INSTDIR\Status.exe" $R1
...
Section "Status Desktop" SecMain
  ...
  WriteRegStr SHCTX "Software\Status.im\Status Desktop" "" $INSTDIR
  WriteRegStr SHCTX "Software\Status.im\Status Desktop" "Version" "${VERSION}"

  WriteRegDWORD SHCTX "Software\Status.im\Status Desktop" "Updated" $PERFORM_UPDATE

  ${StrRep} $R0 "$INSTDIR\uninstall.exe" '"' '""'
  WriteRegExpandStr SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "UninstallString" '"$R0"'
  WriteRegExpandStr SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "InstallLocation" "$INSTDIR"
  WriteRegStr   SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "DisplayName"    "Status Desktop ${VERSION}"
  WriteRegStr   SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "DisplayIcon"    "$INSTDIR\Status.exe"
  WriteRegStr   SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "DisplayVersion" "${VERSION}"
  WriteRegStr   SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "URLInfoAbout"   "${WEBSITE_URL}"
  WriteRegStr   SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "URLUpdateInfo"  "${WEBSITE_URL}"
  WriteRegStr   SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "HelpLink"       "${WEBSITE_URL}"
  WriteRegStr   SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "Publisher"      "${PUBLISHER}"
  WriteRegDWORD SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "VersionMajor"   "${VERSION_MAJOR}"
  WriteRegDWORD SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "VersionMinor"   "${VERSION_MINOR}"
  WriteRegDWORD SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "NoModify"       "1"
  WriteRegDWORD SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "NoRepair"       "1"

  Call GetInstalledSize
  WriteRegDWORD SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "EstimatedSize" "$GetInstalledSize.total" ; Create/Write the reg key with the dword value
  ...
  CreateShortCut "$SMPROGRAMS\$STARTMENU_FOLDER\Status.lnk" "$INSTDIR\Status.exe"

  !insertmacro MUI_STARTMENU_WRITE_END

  !insertmacro SnoreShortcut "$SMPROGRAMS\$STARTMENU_FOLDER\Status.lnk" "$INSTDIR\Status.exe" "StatusIm.Status.Desktop.1" ; app.id must match ID in modules/react-native-desktop-notification/desktop/desktopnotification.cpp
  ...
  ${If} $ALL_USERS == 1
    ...
    ${If} ${RunningX64}
      SetRegView 64
    ${EndIf}
    WriteRegDWORD HKLM "${DUMP_KEY}\Status.exe"        "DumpType" "1"
    WriteRegDWORD HKLM "${DUMP_KEY}\ubuntu-server.exe" "DumpType" "1"
    ${If} ${RunningX64}
      SetRegView lastused
    ${EndIf}
  ${EndIf}
  ...
  WriteRegStr SHCTX "Software\Microsoft\Windows\CurrentVersion\App Paths\Status.exe" "" "$INSTDIR\Status.exe"
  WriteRegStr SHCTX "Software\Microsoft\Windows\CurrentVersion\App Paths\Status.exe" "Path" "$INSTDIR"
...
Function PageFinishPre
  ${If} $PERFORM_UPDATE == 1
    !insertmacro UAC_AsUser_ExecShell '' "$INSTDIR\Status.exe" '' '' SW_SHOWNORMAL
    Quit
  ${EndIf}
FunctionEnd
...
Function un.ReadPreviousVersion
  ...
  ReadRegStr $R0 HKLM "Software\Status.im\Status Desktop" ""
 
  ${If} $R0 != ""
    ;Detect version
    ReadRegStr $R2 HKLM "Software\Status.im\Status Desktop" "Version"
    ${If} $R2 == ""
      StrCpy $R0 ""
    ${EndIf}
  ${EndIf}
 
  ReadRegStr $R1 HKCU "Software\Status.im\Status Desktop" ""
 
  ${If} $R1 != ""
    ;Detect version
    ReadRegStr $R2 HKCU "Software\Status.im\Status Desktop" "Version"
    ${If} $R2 == ""
      StrCpy $R1 ""
    ${EndIf}
  ${EndIf}
...
Function un.RemoveStartmenu
  ...
  Delete "$SMPROGRAMS\$MUI_TEMP\Status.lnk"
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\.env"
  Delete "$INSTDIR\*.dll"
  Delete "$INSTDIR\Status.exe"
  Delete "$INSTDIR\reportApp.exe"
  Delete "$INSTDIR\vc_redist.x64.exe"
  Delete "$INSTDIR\ubuntu-server.exe"
  ...
  ${If} ${Errors}
    ...
    DeleteRegValue SHCTX "Software\Status.im\Status Desktop" "Package"
    DeleteRegValue SHCTX "Software\Status.im\Status Desktop" "Updated"
	  DeleteRegValue SHCTX "Software\Status.im\Status Desktop" "Channel"
  ${EndIf}
  ...
  ${If} $un.REMOVE_ALL_USERS == 1
    ...
    DeleteRegKey /ifempty HKLM "Software\Status.im\Status Desktop"
    DeleteRegKey /ifempty HKLM "Software\Status.im"
    DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop"
  
    Delete "$DESKTOP\Status.lnk"

    ; Remove dump key
    ${If} ${RunningX64}
      SetRegView 64
    ${EndIf}
    DeleteRegValue HKLM "${DUMP_KEY}\Status.exe"            "DumpType"
    DeleteRegValue HKLM "${DUMP_KEY}\ubuntu-server.exe"     "DumpType"
    ${If} ${RunningX64}
      SetRegView lastused
    ${EndIf}
  ${EndIf}
    ...
    ${If} $un.REMOVE_CURRENT_USER == 1
      ...
      DeleteRegKey /ifempty HKCU "Software\Status.im\Status Desktop"
      DeleteRegKey /ifempty HKCU "Software\Status.im"
      DeleteRegKey HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop"
  
      Delete "$DESKTOP\Status.lnk"
    ${EndIf}
  
  DeleteRegKey SHCTX "Software\Microsoft\Windows\CurrentVersion\App Paths\Status.exe"
