...
;  !include "Memento.nsh"
  ...
  !include "3rd-party/SnoreNotify.nsh"
...
Section "Status Desktop" SecMain
  ...
  File "${top_srcdir}\.env"
  File "${top_srcdir}\deployment\windows\nsis\3rd-party\SnoreToast.exe"
  ...
  WriteRegStr   SHCTX "Software\Microsoft\Windows\CurrentVersion\Uninstall\Status Desktop" "DisplayIcon"    "$INSTDIR\${AppExeName}"
  ...
  !insertmacro SnoreShortcut "$SMPROGRAMS\$STARTMENU_FOLDER\${AppLinkFileName}" "$INSTDIR\${AppExeName}" "StatusIm.Status.Desktop.1" ; app.id must match ID in modules/react-native-desktop-notification/desktop/desktopnotification.cpp
...
Function PageFinishPre
  ${If} $PERFORM_UPDATE == 1
    !insertmacro UAC_AsUser_ExecShell '' "$INSTDIR\${AppExeName}" '' '' SW_SHOWNORMAL
    Quit
  ${EndIf}
FunctionEnd
...
Function CustomFinishRun
  ...
  ;UAC::Exec '' '"$INSTDIR/${AppExeName}"' '' ''
  !insertmacro UAC_AsUser_ExecShell '' "${AppExeName}" '' "$INSTDIR" SW_SHOWNORMAL
